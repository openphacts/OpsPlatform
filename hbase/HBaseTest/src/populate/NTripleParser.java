...
public class NTripleParser {
	...
	public static void parse(String file) throws IOException {
		try {
			...
			HBaseUtil.cachePredicates();
			
			// populate table
			for (Iterator<Statement> iter = myList.iterator(); iter.hasNext();) {
				Statement s = iter.next();
				if (s.getObject() instanceof Resource){
					HBaseUtil.addRow(tableName, s.getSubject().toString(), s.getPredicate().toString(), "", s.getObject().toString());
				}
				else {
					HBaseUtil.addRow(tableName, s.getSubject().toString(), "literal", s.getPredicate().toString(), s.getObject().toString());
				}
			}
		}
		catch (Exception e) {
